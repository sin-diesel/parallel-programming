    ...
    for (int row = 0; row < N; row++) {
        for (int col = 0; col < N; col += 8) {
            ...
            for (int idx = 0; idx < N; idx++) {
                __m256i bc_mat1 = _mm256_set1_epi32(mat1[row][idx]);
                __m256i vec_mat2 = _mm256_loadu_si256((__m256i*)&mat2[idx][col]);
                __m256i prod = _mm256_mullo_epi32(bc_mat1, vec_mat2);
                sum = _mm256_add_epi32(sum, prod);
            }
        _mm256_storeu_si256((__m256i*)&result[row][col], sum);
        }
